...
//
int tofInit(PIO pio_i2c, uint sm_i2c, int iAddr, int bLongRange)
{
#ifndef PICO_BOARD
char filename[32];
...
#else
    pio_i2c_write_blocking(vl_pio, vl_sm,vl_address,&ucAddr,1);
    pio_i2c_read_blocking(vl_pio, vl_sm,vl_address,ucTemp,2);
#endif
...
#else
    pio_i2c_write_blocking(vl_pio, vl_sm,vl_address,&ucAddr,1);
    pio_i2c_read_blocking(vl_pio, vl_sm,vl_address,&ucTemp,1);
#endif
...
#else
   pio_i2c_write_blocking(vl_pio, vl_sm,vl_address,&ucAddr,1);
   pio_i2c_read_blocking(vl_pio, vl_sm,vl_address,pBuf,iCount);
#endif
} /* readMulti() */
...
int rc;
	...
	if (rc != iCount+1) {};
#else
   pio_i2c_write_blocking(vl_pio, vl_sm,vl_address,ucTemp,iCount+1);
#endif
} /* writeMulti() */
//
...
#else
   pio_i2c_write_blocking(vl_pio, vl_sm,vl_address,ucTemp,3);
#endif
...
#else
   pio_i2c_write_blocking(vl_pio, vl_sm,vl_address,ucTemp,2);
#endif
...
int rc;
	while (ucCount)
	{
		...
		if (rc != 2) {};
#else
       pio_i2c_write_blocking(vl_pio, vl_sm,vl_address,ucList,2);
#endif
		ucList += 2;
		ucCount--;
	}
